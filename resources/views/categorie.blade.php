...
<div class="container">
                    <div class="browse-slider owl-theme owl-carousel">
                        @foreach(App\Models\Categorie::all() as $x)
                        <a href="{{route('showProductCategories',['id'=>$x->id])}}" class="browse-item">
                            <img src="{{asset($x->path_logo)}}" alt="auction">
                            <span class="info"> <b> {{ $x->nom}}</b> </span>
                        </a>
                        @endforeach
                 ...
                 @if($categorie->products->count()==0) <h2 class="container">{{$categorie->nom}}({{$categorie->products->count()}})</h2>@endif
                    @if($categorie->products->count()!=0)
